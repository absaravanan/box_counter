...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    is_coco = data.endswith('coco.yaml') and nc == 2  # COCO dataset
...
def parse_opt(known=False):
    ...
    parser.add_argument('--project', default='visionModules/objectDetection/yolov5/runs/train', help='save to project/name')
...
def run(**kwargs):
    ...
    for k, v in kwargs.items():
        setattr(opt, k, v)
    saved_dir, results = main(opt)
    saved_dir = saved_dir.replace("visionModules/objectDetection/yolov5/", "")
    current_dir = os.path.dirname(os.path.realpath(__file__))
    model_path = os.path.join(current_dir, saved_dir, "weights/best.pt")

    accuracy = {"precision": results[0],
                "recall": results[1] 
                }
